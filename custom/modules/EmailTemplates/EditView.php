...
if($has_campaign || $inboundEmail) {
    ...
    // TAWSE - we seem to use this for standard Edit View
    // replace with custom
...
if(true) {
    ...
    // TAWSE - Assign additional parameters for Parent Dropdown field
    global $moduleList;
    $parent_dropdown = '<option value=""></option>';
    foreach ($moduleList as $module) {
        $selected = '';
        if ($focus->parent_module_c == $module) {
            $selected = 'selected';
        }
        $parent_dropdown .= '<option value="'.$module.'" '.$selected.'>'.$app_list_strings['moduleList'][$module].'</option>';
    }
    $xtpl->assign("PARENT_MODULE_DROPDOWN", $parent_dropdown);
    // TAWSE
